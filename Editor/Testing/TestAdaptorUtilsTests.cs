...
namespace Hackerzhuli.Code.Editor.Testing
{
    ...
    public class TestAdaptorUtilsTests
    {
        ...
        private async System.Threading.Tasks.Task<List<ITestAdaptor>> RetrieveTestsFromApiAsync(TestMode testMode)
        {
            ...
            while (!_testRetrievalComplete && DateTime.Now < timeoutTime)
            {
                await System.Threading.Tasks.Task.Delay(50); // Wait 50ms between checks
            }

            if (!_testRetrievalComplete)
            {
                Assert.Fail("Test retrieval timed out");
            }
        ...
        [Test]
        public async System.Threading.Tasks.Task GetNodeType_ConcreteTestNodes_ValidateSpecificExamples()
        {
            ...
            // Define expected node types for specific test nodes from our sample test class
             var expectedNodeTypes = new Dictionary<string, TestNodeType>
             {
                 ...
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleTestMethod1", TestNodeType.Method },
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleTestMethod2", TestNodeType.Method },
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleAsyncTest", TestNodeType.Method },
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleCoroutineTest", TestNodeType.Method },
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleParameterizedTest", TestNodeType.Method },

                 
                 // TestCase nodes (parameterized tests)
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleParameterizedTest(\"param1\")", TestNodeType.TestCase },
                 { "Hackerzhuli.Code.Editor.Testing.SampleTestClassForValidation.SampleParameterizedTest(\"param2\")", TestNodeType.TestCase }
             };
            ...
            foreach (var nodeName in validatedNodes)
            {
                Debug.Log($"  ✓ {nodeName}: {expectedNodeTypes[nodeName]}");
            }
